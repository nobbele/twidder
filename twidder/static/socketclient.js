...
class SocketClient {
  ...
  constructor() {
    ...
    this.addListener("OPEN", () => {
      ...
      this.addListener("PONG", (data) => {
        if (data != 0xdeadbeef) console.warn("Invalid PONG response.");
  ...
  open() {
    ...
    this.socket.onclose = (e) =>
      this.close(`onclose(${e.reason || "no reason..."})`);
  }
  ...
  send(message) {
    if (
      ...
      this.lastRequestTimestamp - Date.now() > 30 * 1000
    ) {
  ...
  onMessage(event) {
    ...
    if (!assertSocketServerMessage(msgData)) {
      console.error(
        `Server response was malformed, expected SocketServerMessage. Received ${event.data}`
      );
  ...
  logout = () =>
    this.handleMessage("LOGOUT", { reason: "Logged out by user." });

  login() {
    if (this.token == undefined) throw new Error("Token is undefined.");
    if (!this.isOpen || this.isLoggedIn || this.isLoggingIn) return;
