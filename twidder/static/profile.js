...
const getAccountButton = () =>
  NonNull(getAccountEntry().querySelector("button"));
...
const getDetailsFirstName = () =>
  NonNull(document.getElementById("details-firstname"));
const getDetailsFamilyName = () =>
  NonNull(document.getElementById("details-familyname"));
const getDetailsGender = () =>
  NonNull(document.getElementById("details-gender"));
const getDetailsCity = () => NonNull(document.getElementById("details-city"));
const getDetailsCountry = () =>
  NonNull(document.getElementById("details-country"));

const getMessageWallList = () =>
  NonNull(document.getElementById("wall-message-list"));
...
const refreshMessageWall = async () => {
  ...
  const messagesHtml = messages
    .map(
      (message) => `<li>
        ${escapeHtml(message.author)} says <em>${escapeHtml(
        message.contents
      )}</em>! ${message.region ? `<br /><i>from ${message.region}</i>` : ""}
    </li>`
    )
    .join("<hr />");
...
const getMessageWallPostContents = () =>
  AsHTMLInputElement(NonNull(document.getElementById("message-form-content")));
const getMessageWallForm = () =>
  NonNull(document.getElementById("message-form"));
const getMessageWallFormServerMessage = () =>
  AsHTMLElement(NonNull(getMessageWallForm().querySelector(".form-message")));
const getMessageWallRefresh = () =>
  NonNull(document.getElementById("message-wall-refresh"));
...
const getBrowseEmailInput = () =>
  AsHTMLInputElement(document.getElementById("browse-email"));
const getBrowseFormMessage = () =>
  AsHTMLElement(getBrowseForm().querySelector(".form-message"));
...
const onBrowseViewLoad = () => {
  getBrowseForm().addEventListener("submit", async (e) => {
    ...
    if (success) setActiveTab("home", true);
...
const getChangePasswordForm = () =>
  NonNull(document.getElementById("change-password-form"));
const getChangePasswordOldPasswordInput = () =>
  AsHTMLInputElement(document.getElementById("old-password"));
const getChangePasswordNewPasswordInput = () =>
  AsHTMLInputElement(document.getElementById("new-password"));
const getChangePasswordRepeatNewPasswordInput = () =>
  AsHTMLInputElement(document.getElementById("repeat-new-password"));

const getChangePasswordMessage = () =>
  AsHTMLElement(getChangePasswordForm().querySelector(".form-message"));
