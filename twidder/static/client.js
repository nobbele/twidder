...
const views = {
  welcome: {
    getView: () => getView("welcome-view"),
    onLoad: onWelcomeViewLoad,
  },
  profile: {
    getView: () => getView("profile-view"),
    onLoad: onProfileViewLoad,
  },
};
...
const showToast = (message, type = "INFO", twoline = false) => {
  ...
  if (existingToastTimeout != undefined) clearTimeout(existingToastTimeout);
  ...
  if (twoline) toastEl.classList.add("twoline");
